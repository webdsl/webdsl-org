module message/page
  ...
  define ignore-access-control templateSuccess(messages : List<String>){
    block()[style := "text-align: center;display := block; width: 700px; margin-left: auto ; margin-right: auto ; margin-bottom: 5px; border: 1px solid #BB8800;"]{
  ...
  define ignore-access-control errorTemplateInput(messages : List<String>){
    block()[style := "clear:left; float:left; border: 1px solid #FF0000; margin: 5px 5px 5px -5px; padding: 4px 8px 4px 4px;"]{
      validatedInput
      for(ve: String in messages){
